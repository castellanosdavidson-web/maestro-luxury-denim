    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!productId && saved?.id) {
          ...
          fetchProducts();
          alert("✓ Producto creado. Ahora puedes subir fotos adicionales de galería.");
        } else {
          closeModal();
          fetchProducts();
        }
              ...
              {!currentId && (
                <div className="border border-dashed border-maestro-bone/10 p-4 text-center">
                  <p className="text-[9px] text-maestro-bone/25 tracking-wide">
                    Primero crea el producto → luego aparece aquí la sección de galería para subir fotos adicionales
                  </p>
                </div>
              )}
