...
export default function AdminSuccessCases() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      showToast(editing ? "Caso actualizado ✓" : "Caso creado ✓");
      setShowModal(false);
      fetchCases();
    } else {
      ...
      <header className="border-b border-white/5 px-8 py-5 flex items-center justify-between sticky top-0 bg-[#0a0a0a]/95 backdrop-blur z-50">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] tracking-widest uppercase text-white/30 mt-0.5">Casos de Éxito — {cases.length} registros</p>
          </div>
        </div>
        <button
          onClick={openCreate}
          ...
          <div className="bg-[#111] border border-white/10 w-full max-w-2xl">
            ...
            <form onSubmit={handleSubmit} className="px-8 py-8 space-y-6">
                ...
                >
                  {imagePreview ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-44 gap-3 text-white/20">
                      ...
                      <p className="text-[10px]">JPG, PNG, WEBP — Recomendado: formato vertical 3:4</p>
                    </div>
                  )}
                </div>
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-white/20 mt-1">{form.quote.length} caracteres — Recomendado: entre 80 y 180</p>
              </div>
